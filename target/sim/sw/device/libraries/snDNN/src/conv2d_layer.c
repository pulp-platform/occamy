        ...
        for (uint32_t _oh = 0; _oh < l->OH; _oh++) {
          ...
          if (snrt_is_dm_core()) {
            uint32_t n_ifmap_pixel_read =
                min(compute_num + l->FW - 1, l->IW - ow + (l->pad << 1));
            ...
            if (!l->cluster2cluster || _oh == 0 ||
              ...
              for (uint32_t fh = 0; fh < l->FH; fh++) {
                ...
                if (oh + fh < l->pad || oh + fh >= l->IH + ((l->FH - 1) >> 1)) {
                  dma_memset(
                      &ifmap[write_buf * ifmap_stride + fh * ifmap_row_stride],
                      0, sizeof(double) * l->TILE_CI * n_ifmap_pixel_read);
                } else {
                  uint32_t padding_left = (ow < l->pad) ? (l->pad - ow) : 0;
                  uint32_t padding_right = (ow + compute_num + l->pad <= l->OW)
                                               ? 0
                                               : n_ifmap_pixel_read -
                                                     ((l->FW - 1) >> 1) -
                                                     (l->IW - ow);
                  ...
                  snrt_dma_txid_t ifmap_txid = snrt_dma_start_2d(
                      &ifmap[write_buf * ifmap_stride + fh * ifmap_row_stride +
                             padding_left * ifmap_col_stride], /* dst
                                                                */
                      (double *)&l->ifmap[((oh + fh - l->pad) * l->IW + ow -
                                           (l->pad - padding_left)) *
                                              l->CI +
                                          ci],     /* src */
                      sizeof(double) * l->TILE_CI, /* size */
          ...
          if (snrt_is_compute_core()) {
            ...
            if (ow + compute_id < l->OW) {
              ...
              if (ci != 0 && l->TILE_CI != l->CI) {
                ...
                gemm_fp64_opt(1, 8, l->FH * l->FW * l->TILE_CI,
                              &im2col[read_buf * im2col_mat_stride +
                                      compute_id * im2col_row_stride],
                              0, 0, weights, l->FH * l->FW * l->TILE_CI + 1, 1,
