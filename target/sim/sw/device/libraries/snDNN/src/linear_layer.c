...
void linear_layer(const linear_layer_t *l) {
  ...
  if (snrt_is_compute_core() && snrt_cluster_compute_core_num() < compute_num) {
    ...
    linear_fp32(ifmap, ldI, &weights[offW], ldW, &bias[compute_id], ldB, ofmap,
                ldO, l->CI, l->CO / compute_num, l->CH);
      ...
      for (int j = 0; j < l->CO; j++) {
        if (result[i * l->CO + j] - ofmap[i * l->CO + j] > tolerance) {
          printf("MISMATCH: result[%d][%d] = %f, ofmap[%d][%d] = %f\n", i, j,
                 result[i * l->CO + j], i, j, ofmap[i * l->CO + j]);
          error += 1;
        }
      }
    }
